"""
Face Anonymization — Backend copy

Applies Gaussian blur or pixelation to detected face regions in a frame,
destructively, before the frame is handed to EvidenceStorage. See
app/privacy/config.py for the full scope statement.
"""
...
class FaceAnonymizer:
    ...
    def __init__(self, config: PrivacyConfig, detector: Optional[FaceDetector] = None):
        self.config = config
    ...
    def process(self, frame: np.ndarray) -> Tuple[np.ndarray, int]:
        ...
        Returns:
            Tuple of (processed_frame, faces_anonymized_count). If mode is
            OFF, returns the frame unmodified and count 0.
        """
